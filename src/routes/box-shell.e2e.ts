...
async function activeSaveFileId(page: Page) {
...
async function importPlatinumThroughSaves(page: Page) {
	...
	await expect(page.getByRole('heading', { name: 'Box 02' })).toBeVisible({ timeout: 15000 });
	await page.getByRole('button', { name: 'Next Location' }).click();
	await expect(page.getByRole('heading', { name: 'Box 03' })).toBeVisible({ timeout: 15000 });
...
test('keyboard navigation moves deterministically across the box grid', async ({ page }) => {
	...
	await page.keyboard.press('ArrowUp');
	await expect(page.locator('#collection-control-pane-primary')).toBeFocused();
	await page.keyboard.press('ArrowUp');
	await expect(page.locator('#collection-control-pane-primary')).toBeFocused();
});
...
test('primary and secondary Box Sources follow active Save File selection rules', async ({
	page
}) => {
	await openEmptySaves(page);
	await page.goto('/');
	...
	const emeraldContainer = page
		.locator('.save-card')
		.filter({ hasText: 'emerald-011020251345.sav' });
	const emeraldCard = emeraldContainer.getByRole('button', {
		name: 'Open emerald-011020251345.sav in Boxes'
	});
	const emeraldId = (await emeraldContainer.getAttribute('id'))?.replace('saves-target-', '');
	const bytes = await readFile(emeraldFixturePath);
	await importInput.setInputFiles({
		name: 'second-emerald.sav',
		mimeType: 'application/octet-stream',
		buffer: bytes
	});
	await expect(page.getByText('second-emerald.sav imported and made active.')).toBeVisible({
		timeout: 15000
	});
	const secondId = (
		await page.locator('.save-card').filter({ hasText: 'second-emerald.sav' }).getAttribute('id')
	)?.replace('saves-target-', '');
...
test('Boxes keeps session composition across navigation and restores only the primary view on reload', async ({
	page
}) => {
	await openEmptySaves(page);
	await importEmeraldThroughSaves(page);
	...
	await expect(primary.getByRole('heading', { name: 'Box 02' })).toBeVisible({ timeout: 15000 });
	await primary.getByRole('button', { name: 'Open Box Menu for emerald-011020251345.sav' }).click();
	await page.getByRole('button', { name: 'Open another collection' }).click();
	await page
		.getByRole('dialog', { name: 'Open another collection' })
	...
	await expect(page.locator('.box-pane')).toHaveCount(2);
	await expect(
		page.locator('.box-pane').first().getByRole('heading', { name: 'Box 02' })
	).toBeVisible();
	await primary.getByRole('button', { name: 'Previous Location' }).click();
	...
	await page
		...
		.click();
	await expect(primary.locator('[id$="box-0-slot-0"]')).toHaveAccessibleName(/Carry Move/);
	await primary.getByRole('button', { name: 'Next Location' }).click();
	await expect(primary.getByRole('heading', { name: 'Box 02' })).toBeVisible();
	...
	await expect(page.getByRole('dialog')).toHaveCount(0);
	await expect(page.locator('[aria-label*="Carry Move"]')).toHaveCount(0);
});
...
test('Box Menu exports and backs up the captured secondary Save File Workspace', async ({
	...
	await expect(secondarySlotMenu.getByRole('button', { name: 'Create Pokemon' })).toBeVisible();
	await page.keyboard.press('Escape');
...
test('Box Menu allows duplicate Save File panes and keeps Open another collection disabled at two panes', async ({
	...
	await page.locator('#pane-primary-box-0-slot-0').click();
...
test('Party never paints a fabricated Pokemon while a Box projection is pending', async ({
	page
}) => {
	await installWorkspaceResponseHold(page);
	await openEmptySaves(page);
	...
	await expect(pane).not.toContainText('Pikachu');
	await releaseWorkspaceResponses(page);
	await expect(pane.locator('[id$="party-slot-0"]')).toContainText('1-UP', { timeout: 15000 });
	await expect(pane.locator('[id$="party-slot-0"]')).toContainText('Lv. 25');
});
...
test('two Box Panes keep physical focus and Carry through rotation, mutation, and close', async ({
	...
	await panes.nth(1).getByRole('button', { name: 'Next Location' }).click();
	await expect(panes.nth(1)).toHaveAttribute('data-location', 'box-1');
	await expect(panes.nth(1).getByRole('heading', { name: 'Box 02' })).toBeVisible();
	await expect(panes.nth(1)).not.toHaveAttribute('aria-busy', 'true', { timeout: 15000 });
	await expect(panes.nth(0)).toHaveAttribute('data-location', 'box-0');
	await expect(panes.nth(0).getByRole('heading', { name: 'Box 01' })).toBeVisible();
	await expect(panes.nth(0).locator('[id$="box-0-slot-0"]')).toContainText('ARON');

	await panes.nth(0).locator('[id$="box-0-slot-0"]').click();
	await expect(panes.nth(0)).toHaveClass(/active-pane/);
	...
	await expect(panes.nth(0).locator('[id$="party-slot-0"]')).toContainText('1-UP');
	await expect(panes.nth(1)).toHaveAttribute('data-location', 'box-1');
	await expect(panes.nth(1).locator('[id$="box-1-slot-0"]')).toHaveAccessibleName(/Empty/);

	await panes.nth(0).getByRole('button', { name: 'Next Location' }).click();
	await expect(panes.nth(0)).toHaveAttribute('data-location', 'box-0');
	await panes.nth(1).locator('[id$="box-1-slot-0"]').click();
	...
	await expect(panes.nth(1).locator('[id$="box-0-slot-0"]')).toContainText('ARON');
	await expect(panes.nth(0)).toHaveAttribute('data-location', 'box-0');
	await expect(panes.nth(0).locator('[id$="box-0-slot-0"]')).toContainText('ARON');
	...
	for (const viewport of [
		...
		await setSafeArea(
			page,
			viewport.width === 640 && viewport.height === 360
				? { top: 12, right: 12, bottom: 12, left: 12 }
				: viewport.width === 640 && viewport.height === 480
					? { top: 12, right: 0, bottom: 12, left: 0 }
		...
		if (viewport.width === 640 && viewport.height <= 480) {
			const compact = await page.evaluate(() => {
				...
				return {
					paneWidths: Array.from(document.querySelectorAll<HTMLElement>('.box-pane')).map(
...
test('Edit opens Pokemon Editor and returns focus to the command stack', async ({ page }) => {
	...
	await expect(editor.locator('[data-editor-active="true"]')).toHaveAttribute(
		'aria-label',
		'Nickname Editing'
	);
	await pressController(page, 'PageDown');
...
test('Pokemon Editor keeps staged state through review, Legality, guarded Back, and rotation', async ({
	...
	const editor = page.getByRole('dialog', { name: 'ARON' });
	await expect(editor).toBeVisible();
	await expect(page.locator('.takeover-frame')).toHaveCSS('width', '616px');
	await expect(page.locator('.takeover-frame')).toHaveCSS('height', '336px');
	await expect(page.locator('.editor-rail')).toHaveCSS('width', '148px');

	await choosePokemonEditorSection(page, 'stats');
	const dimensions = await editor.evaluate((dialog) => {
		const editorElement = dialog.querySelector<HTMLElement>('.pokemon-editor')!;
		const input = dialog.querySelector<HTMLElement>('#pokemon-editor-hp-iv')!;
		const label = input.closest('label')!.querySelector<HTMLElement>('span')!;
		const sectionLabel = dialog.querySelector<HTMLElement>(
			'[aria-label="IV and EV Editing"] .panel-title span'
		)!;
		return {
			...
			columnHeaderSize: parseFloat(getComputedStyle(label).fontSize),
			labelSize: parseFloat(getComputedStyle(sectionLabel).fontSize)
		};
	});
	expect(dimensions.editorScrollWidth).toBeLessThanOrEqual(dimensions.editorClientWidth);
	expect(dimensions.controlHeight).toBe(32);
	expect(dimensions.columnHeaderSize).toBe(10);
	expect(dimensions.labelSize).toBe(12);
	...
	await expect(page.locator('.takeover-frame')).toHaveCSS('width', '760px');
	await expect(page.locator('.takeover-frame')).toHaveCSS('height', '560px');
...
test('Pokemon Editor changes Held Item and returns focus to the command stack', async ({
	...
	const editor = page.getByRole('dialog', { name: 'ARON' });
	await choosePokemonEditorSection(page, 'held-item');
...
test('Pokemon Editor changes Ability and returns focus to the command stack', async ({ page }) => {
	...
	const editor = page.getByRole('dialog', { name: 'ARON' });
	await choosePokemonEditorSection(page, 'ability');
...
test('Pokemon Editor changes level through Apply and keeps editor focus', async ({ page }) => {
	...
	await expect(editor.locator('#pokemon-editor-species')).toBeEnabled({ timeout: 15000 });
	await choosePokemonEditorSection(page, 'level-experience');
	await expect(editor.locator('[aria-label="Level and Experience Editing"]')).toContainText(
		'Current level 11'
	);
...
test('Pokemon Editor stages and applies Friendship while restoring focus', async ({ page }) => {
	...
	const editor = page.getByRole('dialog', { name: 'ARON' });
	await choosePokemonEditorSection(page, 'friendship');
...
test('Backup Browser owns active Save File recovery, fresh focus, guarded Back, and floor geometry', async ({
	...
	await expect(
		page.getByRole('button', { name: /Open Box Menu for .*011020251345\.restored\.sav/ })
