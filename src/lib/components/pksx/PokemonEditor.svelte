<script lang="ts">
	...
	let draftOriginalTrainerName = $state(
		untrack(() => originalTrainerEditConstraints?.currentName ?? '')
	);
	let draftTrainerId = $state(
		untrack(() => String(originalTrainerEditConstraints?.currentTrainerId ?? 0))
	);
	let draftSecretId = $state(
		untrack(() => String(originalTrainerEditConstraints?.currentSecretId ?? 0))
	);
	let draftTrainerGenderId = $state(
		untrack(() => originalTrainerEditConstraints?.currentGenderId ?? 0)
	);
	let draftLanguageId = $state(
		untrack(() => originalTrainerEditConstraints?.currentLanguageId ?? 0)
	);
