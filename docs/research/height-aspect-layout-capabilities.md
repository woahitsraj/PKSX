...
- **`prerender = true` means the HTML is generated at build time.** There is no server that sees a
  request, let alone a viewport. Any layout decision made in JS is *necessarily wrong* in the
...
that is the *large* viewport** — i.e. it behaves like `lvh`, not `dvh`. It does not track a
...
`100dvh` box's notion of "height" can disagree by the toolbar height in a *browser* tab. Inside a
...
for `svh`/`lvh`/`dvh`, but treat it as *strongly-held convention, not cited spec* until verified.
...
> **Note:** This feature does not correspond to *device* orientation. Opening the soft keyboard on
...
**15.0**, so an iOS 15 or iOS 16.0–16.3 device would *not* parse range syntax and would drop those
...
exists only in *units*, not in *media features*. If the spec needs to branch on "how much height do
...
requires no JS. **However**, I found no primary source that pins down *ordering* between the media
...
`@container (height < 500px)` are all *syntactically* available. Support: Chrome **105** (2022-08-30),
...
to reflect the *viewport*, the chain has to be anchored at the top by something viewport-aware —
...
Yes, *if* the container's dimensions actually change. A container query is invalidated by layout,
...
**Evidence gap:** I found no primary source describing container query invalidation *timing* during
...
container, and without size containment. It is the CSS-only answer to the problem container *size*
...
Per MDN, `vh` currently resolves equal to `lvh` in all browsers. Note `100lvh` can therefore *exceed*
...
other only when the user agent has *retractable interface chrome* (a collapsing URL bar). A
...
**The exception is the keyboard.** A soft keyboard *can* change the WebView's height, and whether it
...
**However** — and this is the honest version — the *safe-area* half of the same problem is
...
indicator, and `100dvh` is the full extent. Safe-area insets must be applied as *padding inside*
...
- **Portrait**: `top` is large (status bar + notch), `bottom` is the home indicator, `left`/`right`
  are ~0.
- **Landscape**: `left` **or** `right` is large (whichever side the notch rotated to — it *swaps*
...
**Evidence gap:** I did not find an Apple or WebKit document giving the *numeric* inset values per
...
**Design implication:** a layout whose *structure* depends on an inset value will visibly reflow on
launch and on rotation. A layout that uses insets only for *padding* — the `max(18px, env(...))`
...
- Ionic hit a *regression* in the other direction after a recent Android System WebView update
...
**Evidence gap:** whether Capacitor 8.4.2 *already* injects `--safe-area-inset-*` without explicitly
...
- **No height variants of any kind.** No `min-h-screen:`-style variant, no aspect-ratio variant.
  (The `aspect-*` *utilities* exist; the *variant* does not.)
...
the utility classes — *if* Tailwind processes Svelte `<style>` blocks.
...
1. **`fallback` is server-only.** The server build is literally
   ...
   ([index-server.js](https://unpkg.com/svelte@5.55.5/src/reactivity/index-server.js)). The *client*
...
**Verdict:** same *category* of problem as `bind:innerWidth` (the server cannot know), but strictly
...
This is the only mechanism that measures a *real box* rather than the viewport, so it is the honest
...
are invoked as part of the HTML spec's *update the rendering* step
...
*"ResizeObserver loop limit exceeded"* is legacy Chrome wording. Both appear in the wild; if the spec
...
Because they run *in an effect*, they are client-only. Falsy values are no-ops
(`{@attach enabled && myAttachment}`), which gives clean conditional wiring. Reactivity trap:
`{@attach foo(bar)}` re-runs on any change to `foo` *or* `bar`.
...
**Ordering: no primary source found for a cross-API guarantee.** What *is* verified is that these all
land in the same frame, inside one *update the rendering* pass, before paint. So the practical rule
...
the source. A CSS-only solution has *zero* correction, because the browser applies the media query
...
1. **Do Tailwind v4 directives (`@variant`, `@apply`, custom variants) work inside Svelte component
   ...
   whether band definitions can be shared or must be duplicated. *~10 minutes to test.*
2. **Does `@custom-variant` with a `@media` + `@slot` body compile correctly** in Tailwind 4.3.0?
   Docs only demonstrate `@supports`. *~5 minutes.*
3. **Does Capacitor 8.4.2 inject `--safe-area-inset-*` without explicitly adding the SystemBars
   plugin**, and are the injected values re-dispatched on rotation? *Measure on device via the
   existing `scripts/run-android-acceptance.sh`.*
4. **Does `env(safe-area-inset-*)` go stale across rotation in current WKWebView** (as opposed to
   ...
   is against UIWebView and reported fixed. *Measure via `scripts/run-ios-acceptance.sh`.*
5. **Do `dvh`/`svh`/`lvh` recompute correctly on the first frame after rotation in WKWebView and
   ...
   safe-area machinery is known-flaky. *Measure on device.*
6. **Confirm `dvh == svh == lvh == vh` inside both WebViews.** Follows from the spec definitions but
   ...
   branching. *One line of CSS + a screenshot.*
...
11. **Cross-API event ordering on rotation** (`matchMedia` vs `resize` vs `ResizeObserver` vs
    ...
    *update the rendering* pass; **no source gives a relative order.** Do not depend on one.
...
- Media query **range syntax** → iOS 16.4. Avoidable by using `min-`/`max-` form. *(Though Tailwind
  already emits range syntax for its own breakpoints, so the floor is arguably already raised.)*
...
- **Nothing in the axis model itself requires JS.** Height, aspect ratio, and orientation are all
  ...
  genuinely available.
- Three things *would* force a JS escape hatch, in ascending order of likelihood:
...
padding wrong for under a second recovers invisibly; a layout that picks a different *structure*
...
### 1. WKWebView's layout viewport is transiently wrong *during* rotation — open for 9 years
...
**Why this matters here specifically.** `height`, `width`, `aspect-ratio` and `orientation` media features are *all* computed from the layout viewport. A transient **320×320** layout viewport evaluates `orientation` to **portrait** — because the spec defines portrait as `height >= width`, so a square viewport is portrait. Mid-rotation, a portrait/landscape binary switch can therefore evaluate to the wrong branch.

Two caveats, stated because the evidence does not go further:

- The bug is written entirely in terms of JS `innerWidth`/`innerHeight`. **No comment discusses CSS or media queries.** Whether this produces a *visible* transient CSS flash, or is merely an internal intermediate state before final layout, is not determinable from the source.
...
**Design consequence:** prefer thresholds that degrade gracefully over a hard portrait/landscape binary that visibly restructures the DOM. A design that merely *adjusts* across a square viewport is safe; one that *swaps layouts* at exactly 1:1 is not.
...
So `svh`/`lvh`/`dvh` exist as *units* but have no *media-feature* counterpart, and none is coming soon. `@media (height: …)` always means the **layout viewport** — in Chrome Android that equals the small viewport; it is never the dynamic viewport in either engine. The gap is acknowledged and unaddressed.
